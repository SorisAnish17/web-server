...
import { ResponseMessageDto } from '../../../../core/dto/schedule-unread-messages/response';
  ...
  async createScheduleUnreadMessages(
    ...
    try {
      ...
      if (existingMessage) {
        ...
        return {
          message: 'Unread message count incremented successfully',
          success: false,
        };
      } else {
        ...
        if (result.insertedId) {
          return {
            message: 'Scheduled unread message stored successfully',
            success: true,
          };
        } else {
