...
export class ChatEventCollection {
  ...
  async sendMessage(messageData: CreateChatMessageDto) {
    try {
      ...
      console.log('coming messages from the api', messageData);
  ...
  async viewMessage(userId: string, messageId: string) {
    try {
      ...
      const message = await connectionDB.findOne({ _id: messageIdObject });
      if (!message) {
      ...
      await connectionDB.updateOne(
        { _id: messageIdObject },
        { $set: { readBy: message.readBy, updatedAt: message.updatedAt } },
      );
