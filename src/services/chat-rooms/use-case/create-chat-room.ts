...
const filterTicket = async (referenceInfo: ReferenceInfo) => {
  ...
  const { category, issueReporter, issueWithUser } = ticket;

  console.log('issueReporter', issueReporter);

  console.log('issueWithUser', issueWithUser);

  if (category && category.toLowerCase() === 'order' && issueWithUser) {
    participants.push({
      organisationId: issueWithUser.companyId,
  ...
  participants.push({
    organisationId: issueReporter.companyId,
